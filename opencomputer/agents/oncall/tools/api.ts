...
export const replayRequest = defineTool({
  name: "replay_request",
  description: "Run the captured request against repository/app/api.mjs in the cloud checkout, using a fresh Node process. Optionally choose another captured reportId to check healthy behavior. Returns actual response or exception evidence; an application error is evidence, not a tool failure.",
  input: { type: "object", properties: { reportId: { type: "string" } }, additionalProperties: false },
  async run({ input, signal }) {
    const incident = await loadIncident("api");
    const snapshot = incident.snapshot as ApiSnapshot;
    ...
    return replay("api", snapshot, checkout, signal);
  },
});
